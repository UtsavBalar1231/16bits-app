...
//COLOR.a = 
//uniform vec4 color = vec4(R, 0.0, 0.3, 0.5);
...
void fragment()
{
	...
	//AQUI DEFINE A TRANSPARENCIA
	float trans = tex.a / transparencia;
	...
	if (enable_palette_cycling)
	{
		...
		COLOR = vec4(texture(palette, vec2(palette_swap, 0)).rgb, trans);
		
	}
	else COLOR = vec4(tex.rgb, trans);
